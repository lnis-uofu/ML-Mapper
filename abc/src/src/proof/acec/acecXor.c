...
void Gia_edgelist(Gia_Man_t * p, char *f0, char *f1, char *f2)
{
    ...
    // fix to print the edges polarity: 1 if inverted (negative polarity); 0 otherwise
    Gia_ManForEachAnd( p, pObj, i ){
        //printf("%d %d  %d\n", Gia_ObjFaninId0(pObj, Gia_ObjId(p,pObj)), Gia_ObjId(p,pObj), Gia_ObjFaninC0(pObj));
        fprintf(f_el, "%d %d %d\n", Gia_ObjFaninId0(pObj, Gia_ObjId(p,pObj))-1, Gia_ObjId(p,pObj)-1, Gia_IsComplement(Gia_ObjChild0(pObj)));
        //printf("%d %d  %d\n", Gia_ObjFaninId1(pObj, Gia_ObjId(p,pObj)), Gia_ObjId(p,pObj), Gia_ObjFaninC1(pObj));
        fprintf(f_el, "%d %d %d\n", Gia_ObjFaninId1(pObj, Gia_ObjId(p,pObj))-1, Gia_ObjId(p,pObj)-1, Gia_IsComplement(Gia_ObjChild1(pObj)));
    ...
    // add fix to do not allow a PO as a child 
    Gia_ManForEachCo( p, pObj, i ){
        if(Gia_ObjFaninId0(pObj, Gia_ObjId(p,pObj))-1 != Gia_ObjId(p,pObj)-1 && !Gia_ObjIsCo(Gia_ObjChild0(pObj)))
        {
            fprintf(f_el, "%d %d %d\n", Gia_ObjFaninId0(pObj, Gia_ObjId(p,pObj))-1, Gia_ObjId(p,pObj)-1, Gia_IsComplement(Gia_ObjChild0(pObj))); 
        }
       
        if(Gia_ObjId(p,pObj)-1 != Gia_ObjFaninId1(pObj, Gia_ObjId(p,pObj))-1 && !Gia_ObjIsCo(Gia_ObjChild1(pObj))) 
        {
            fprintf(f_el, "%d %d %d\n", Gia_ObjFaninId1(pObj, Gia_ObjId(p,pObj))-1, Gia_ObjId(p,pObj)-1, Gia_IsComplement(Gia_ObjChild1(pObj))); 
        }

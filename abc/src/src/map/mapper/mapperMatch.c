...
int Map_MatchNodeCut( Map_Man_t * p, Map_Node_t * pNode, Map_Cut_t * pCut, int fPhase, float fWorstLimit )
{
    ...
    // int status = Map_CutPrint_(p, pNode, pCut);
...
//        assert( pCutBest->M[fPhase].tArrive.Fall < pNode->tRequired[fPhase].Fall + p->fEpsilon );
    ...
    {   
        //printf("Mapping cut %u of node %d! Phase is %d\n", mapCounter, pNode->Num, fPhase);
        mapCounter+=1;
...
//        assert( Area2 < Area1 + p->fEpsilon );
    ...
    // printf("Best match for cut %u; idx: %d; with phase %d, is: \n", pCutBest->uTruth, pNode->Num, fPhase);
    // int i; 
    // for ( i = 0; i < pNode->pCutBest[fPhase]->nLeaves; i++ )
    // {
    //     int pNodeChildIdx  = pNode->pCutBest[fPhase]->ppLeaves[i]->Num;
    //     printf( "Child %d: %d\n", i, pNodeChildIdx );
    // }
...
int Map_MappingMatches( Map_Man_t * p )
{
    ...
    //for area flow starts from required timing (given by the delay map)
    ...
    {   
        uint32 cutCount = 0;
        uint32 trivialCutCount = 0;
        pNode = p->vMapObjs->pArray[i];
        // for ( pCut = pNode->pCuts->pNext; pCut; pCut = pCut->pNext )
        // {
        //     if(pCut->nLeaves == 1 || pCut->nLeaves == 0 ){
        //         trivialCutCount += 1;     
        //     }
        //     else
        //     {
        ...
        // }   
        // printf("#########################################################\n");
        // printf("Node index %d! Number of cuts %u! Skipped %u trivial cuts\n", pNode->Num, cutCount, trivialCutCount);

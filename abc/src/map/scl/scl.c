...
***********************************************************************/
void scl_criticalNodesLabel( SC_Man * p, char * fOut)
{
...
void scl_network2edgelist_seq(Abc_Ntk_t * pNtk, SC_Man * p, char * fOut)
{
    //vector<int> vec_id; vec_id = collectId(pNtk);
    //int max_id = *(vec_id.end()-1);
    FILE *fout;
    ...
    Abc_NtkForEachObj(pNtk,pObj,i){
        ...
        Abc_ObjForEachFanout(pObj, pFanout, i2){
            if(Abc_ObjIsLatch(pObj) && !Abc_ObjIsLatch(pFanout)){
                fprintf(fout, "%d %d ", Abc_ObjId(pObj), Abc_ObjId(pFanout));
                fprintf(fout, "%d \n", (int)Abc_SclObjTimeMax(p, pObj));
            }
...
int Scl_CommandStime( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    int matrixFile=0;
    int labelFile=0;
    char * Filename, * labelFilename;
    Extra_UtilGetoptReset();
    while ( ( c = Extra_UtilGetopt( argc, argv, "Xcapdhfl" ) ) != EOF )
    {
        switch ( c )
        {
            case 'X':
            ...
            case 'l':
                ...
                labelFile = 1;
                labelFilename = argv[globalUtilOptind];
                globalUtilOptind++;
                break;
            default:
                goto usage;
    ...
    SC_Man * p; 
    p = Abc_SclManStart( (SC_Lib *)pAbc->pLibScl, pNtk, fUseWireLoads, 1, 0, nTreeCRatio );
    int i4; Abc_Obj_t * pObj;
    if (matrixFile==1)
        scl_network2edgelist_seq( pNtk, p, Filename);
    if (labelFile == 1){
        Vec_Int_t * vPathPos, * vPathNodes;
        vPathPos   = Abc_SclFindCriticalCoWindow( p, 25 ); // 100 - 5 = 95%
        vPathNodes = Abc_SclFindCriticalNodeWindow( p, vPathPos, 25, 0 );
        FILE *fout;
        fout = fopen(labelFilename, "w");

        if (fout == NULL) {
            perror("open failed");
            exit(1);
        }
        Abc_NtkForEachObj(p->pNtk,pObj,i4){
            fprintf(fout, "%d,%d\n", Abc_ObjId(pObj),pObj->fMarkA);
        }

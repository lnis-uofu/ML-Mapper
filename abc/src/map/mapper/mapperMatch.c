...
***********************************************************************/
int Map_MatchNodePhase( Map_Man_t * p, Map_Node_t * pNode, int fPhase, int index )
{
    ...
    //printf("Matching cuts for node %d! Cut index to be matched is %d\n", pNode->Num, index);
    //printf("Cut List:\n");
    //Map_CutListPrint(p, pNode);
    //("\n");
...
//        assert( pCutBest->M[fPhase].tArrive.Fall < pNode->tRequired[fPhase].Fall + p->fEpsilon );
    ...
    // {
    //     cutCounter++; 
    //     Map_CutPrint_(p, pCut, pNode);
    // }
    // //getchar();

    //printf("Node %d has %d cuts\n", pNode->Num, cutCounter);
    int isFeasible = 0; 
    int iterator = 0; 
    //do {
        // if (cutCounter > 1) {
        //     randIndex = (rand() % (cutCounter - 1 + 1)) + 1;
        // }
        // else {
        //     randIndex = 1; 
        // }
        ...
        {  
            ...
            isFeasible = Map_MatchNodeCut( p, pNode, pCut, fPhase, fWorstLimit );
            iterator++;
            //if (isFeasible == 0) {
                //printf("Cant match index: %d!\n", index);
            //    break; 
            //}
            if ( pMatch->pSuperBest == NULL || pMatch->tArrive.Worst > fWorstLimit + p->fEpsilon )
                continue;
            //isFeasible = 1; 
            //printf("Used for map cut index %d (iterator is %d)\n", index, iterator );
            //Map_CutPrint_(p, pCut, pNode);
            //printf("Cut has %d leaves and %d volume. TT is %u!\n", pCut->nLeaves, pCut->nVolume, pCut->uTruth);
            // int i = 0;
...
int Map_MappingMatches( Map_Man_t * p )
{
    ...
    for ( i = 0; i < p->vMapObjs->nSize; i++ )
    {
        ...
        //srand (time (NULL));
        if ( pNode->CutCounter > 0 && cutCounter > 0) {
            // matching based on ML model prediction
            int k = 0; 
            // printf("Node %d has %d choices!!", pNode->Num, pNode->CutCounter);
            // getchar();
            for (k =0; k<pNode->CutCounter; k++){
                // if (k >= 250)
                //     continue;
            
                // match negative phase
                //printf("trying to map cut %d for node %d\n", pNode->CutChoices[k], pNode->Num);
                    if ( !Map_MatchNodePhase( p, pNode, 0, pNode->CutChoices[k] ) )
                    {
                        Extra_ProgressBarStop( pProgress );
                        return 0;
                    }
                    // match positive phase
                    if ( !Map_MatchNodePhase( p, pNode, 1, pNode->CutChoices[k] ) )
                    {
                        Extra_ProgressBarStop( pProgress );
                        return 0;
                    }
        ...
        //default is set now to be the random mapper, so that we can generate data for trainning
        // if (pNode->CutCounter == 0 && cutCounter > 1) {
        //     //printf("random mapper!\n");
        ...
        //             // match negative phase
        //             if ( !Map_MatchNodePhase( p, pNode, 0, randIndex ) )
        //             {
        //                 Extra_ProgressBarStop( pProgress );
        //                 return 0;
        //             }
        //             // match positive phase
        //             if ( !Map_MatchNodePhase( p, pNode, 1, randIndex ) )
        //             {
        //                 Extra_ProgressBarStop( pProgress );
        //                 return 0;
        //             }
        //         } while (pNode->pCutBest[0] == NULL && pNode->pCutBest[1] == NULL); 
        //     }
        // }

        if (cutCounter == 0 || pNode->pCutBest[0] == NULL && pNode->pCutBest[1] == NULL) {
            //printf("just one option for node %d\n", pNode->Num); 
            //getchar();
            // match negative phase
                if ( !Map_MatchNodePhase( p, pNode, 0, 0 ) )
                {
                    Extra_ProgressBarStop( pProgress );
                    return 0;
                }
                // match positive phase
                if ( !Map_MatchNodePhase( p, pNode, 1, 0 ) )
                {
                    Extra_ProgressBarStop( pProgress );
                    return 0;
                }

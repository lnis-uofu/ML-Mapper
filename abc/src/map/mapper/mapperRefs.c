...
{
    Map_NodeVec_t * pStructure; 
    pStructure = Map_NodeVecAlloc( 100 );
    ...
    // printf("Cut used for mapping node %d: \n Truth-table: %u \n phase: %d \n", 
    //       pNodeR->Num, pCut->uTruth, fPhase); 

    // for ( pCutAux = pNodeR->pCuts->pNext; pCutAux; pCutAux = pCut->pNext ){
    //     if(pCutAux == pCut)
    //     {
    //       printf("Cut used for mapping node %d: \n Truth-table: %u \n phase: %d \n", 
    //       pNodeR->Num, pCutAux->uTruth, fPhase); 
    //     }
    // }

    //Map_TruthsCutOne ( pMan, pCut, uTruthAux );

    rootGate = pCut->M[fPhase].pSuperBest->pRoot; 
    // root id, tt, phase, n leaves
    
    //     printf("%d, %u, %d, %d, %d\n", 
    //        pNodeR->Num, Mio_GateReadTruthP( rootGate ), fPhase, pCut->nLeaves, pCut->nVolume); 
    
    //printf("Cut root node: %d\n", pNodeR->Num);

    // Use auxiliar pointer 
    //pNodeAux = pNodeR; 

    // clean visited nodes
    //Map_MappingUnmark_rec( pNodeAux );    

    // mark leaves as visited 
    // for ( i = 0; i < pCut->nLeaves; i++ )
    // {
    //     int pNodeChildIdx  = pCut->ppLeaves[i]->Num;
    //     pCut->ppLeaves[i]->fMark0 = 1;
    //     printf( " Child %d: %d\n", i, pNodeChildIdx );
    // } 
    
    // printf("Nodes covers %c nodes:\n", pCut->nVolume);
    
    // visit cut structure 
    // Map_MappingDfs_rec(pNodeAux, pStructure, 0);
    
    // for (i=0; i<pStructure->nSize; i++)
    // {
    //     printf("%d; ", pStructure->pArray[i]->Num);
    // }

    // printf("\n");
    //printf("Cut TT:\n %s ", Mio_GateReadSop( rootGate ));

    //printf("Cut TT:\n %u ", Mio_GateReadTruth( rootGate ));
    //printf("Cut matched polarity: %d\n", fPhase);
...
void Map_MappingSetRefs( Map_Man_t * pMan )
{
    ...
    for ( i = 0; i < pMan->nOutputs; i++ )
    {
        ...
        //chamando _mod que eh minha funcao modificada. Voltar isso. 
        if ( !Map_NodeIsConst(pNode) )
            Map_MappingSetRefs_rec( pMan, pNode );
    }
}

void Map_MappingPrintCuts_rec( Map_Man_t * pMan, Map_Node_t * pNode )
{
    Map_NodeVec_t * pStructure; 
    pStructure = Map_NodeVecAlloc( 100 );
    ...
    if ( pMan->fUseProfile )
        Mio_GateIncProfile2( pCut->M[fPhase].pSuperBest->pRoot );
    //Map_TruthsCutOne ( pMan, pCut, uTruthAux );
    int minLevel = 1000000, maxLevel = 0, cutLevel = 0; 
    ...
    int relativeLevel = totalLevel - pNodeR->Level;
    //printf("%d,%d,%d,%d,%d,%d,-2,end\n", pNodeR->Num, leavesIdx[0], leavesIdx[1], leavesIdx[2], leavesIdx[3], leavesIdx[4]);
    printf("%d,%d,%u,%u,%u,%u,%d,%u,%u,%d,%u,%d,%d,%d,%d,%d,%d,%d,%d,%d,1000000,%d,%d,%d,%d,%d,%d,%s\n", pNodeR->Num, pNodeR->nRefs, pNodeR->Level, pNodeR->fInv, pNodeR->p1->fInv, pNodeR->p1->Level, pNodeR->p1->nRefAct[2], pNodeR->p2->fInv, pNodeR->p2->Level, pNodeR->p2->nRefAct[2], pCut->M[fPhase].pSuperBest->uTruth, fPhase, pCut->nLeaves, pCut->nVolume, minLevel, maxLevel, cutLevel, minFo, maxFo, cutFo, relativeLevel, leavesIdx[0], leavesIdx[1], leavesIdx[2], leavesIdx[3], leavesIdx[4] , Mio_GateReadName( rootGate ));
    // printf("%d,%u,%s,%d,%d,%d\n", 
    //     pNodeR->Num, Mio_GateReadTruthP( rootGate ), Mio_GateReadName( rootGate ), fPhase, pCut->nLeaves, pCut->nVolume); 
    
    // printf("%d,%d,%u,%u,%u,%u,%u,%u,%s,%u,%d,%d,%d,%f,%f,%d\n", pNodeR->Num, pNodeR->nRefs, pNodeR->Level, pNodeR->fInv, pNodeR->p1->fInv, pNodeR->p1->Level, pNodeR->p2->fInv, pNodeR->p2->Level, Mio_GateReadName( rootGate ), pCut->M[fPhase].pSuperBest->uTruth, fPhase, pCut->nLeaves, pCut->nVolume, pCut->M[fPhase].tArrive.Worst, pCut->M[fPhase].AreaFlow);
    //printf("%s,%u,%d,%d,%d,%f,%f,%d\n",  Mio_GateReadName( rootGate ), pCut->M[fPhase].pSuperBest->uTruth[fPhase], fPhase, pCut->nLeaves, pCut->nVolume, pCut->M[fPhase].tArrive.Worst, pCut->M[fPhase].AreaFlow);
    ...
    {
        fInvPin = ((uPhase & (1 << i)) > 0);
        Map_MappingPrintCuts_rec( pMan, Map_NotCond(pCut->ppLeaves[i], fInvPin) );
    }
}
void Map_MappingPrintCuts( Map_Man_t * pMan )
{
    Map_Node_t * pNode;
    int i;
    if ( pMan->fUseProfile )
    ...
    for ( i = 0; i < pMan->nOutputs; i++ )
    {
        ...
        //chamando _mod que eh minha funcao modificada. Voltar isso. 
        if ( !Map_NodeIsConst(pNode) )
            Map_MappingPrintCuts_rec( pMan, pNode );
    }
}
